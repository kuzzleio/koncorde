...
describe("Koncorde.keyword.match", () => {
  ...
  describe("#validation", () => {
    it("should reject non-object filters", () => {
      should(() => koncorde.validate({ match: ["foo", "bar"] })).throw({
        keyword: "match",
    ...
    it("should reject empty filters", () => {
      should(() => koncorde.validate({ match: {} })).throw({
        keyword: "match",
    ...
    it("should validate filters with object argument", () => {
      should(() =>
        koncorde.validate({ match: { foo: undefined } }),
      ).not.throw();
    });
  ...
  describe("#matching", () => {
    ...
    it("should match undefined equality", () => {
      ...
      should(koncorde.test({ a: undefined }))
        .be.an.Array()
        .length(1);
    });
  });
  ...
  describe("#removal", () => {
    ...
    it("should remove a single subfilter from a multi-filter condition", () => {
      ...
      const id2 = koncorde.register({
        and: [{ match: { baz: "qux" } }, { match: { foo: "bar" } }],
      });
      ...
      should(
        match.custom.filters.findIndex(
          (f) => f.subfilter.id === multiSubfilter.id && f.value.baz === "qux",
        ),
      ).be.greaterThan(-1);
      should(
        match.custom.filters.findIndex(
          (f) => f.subfilter.id === multiSubfilter.id && f.value.foo === "bar",
        ),
      ).be.greaterThan(-1);
    });
  });
