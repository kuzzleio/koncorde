...
describe("Koncorde.keyword.range", () => {
  ...
  describe("#validation", () => {
    it("should reject empty filters", () => {
      should(() => koncorde.validate({ range: {} })).throw({
        keyword: "range",
    ...
    it("should reject filters with more than 1 field", () => {
      should(() =>
        koncorde.validate({ range: { foo: "foo", bar: "bar" } }),
      ).throw({
        keyword: "range",
    ...
    it("should reject an empty field definition", () => {
      should(() => koncorde.validate({ range: { foo: {} } })).throw({
        keyword: "range",
    ...
    it("should reject a field definition containing an unrecognized range keyword", () => {
      should(() =>
        koncorde.validate({ range: { foo: { gt: 42, lt: 113, bar: "baz" } } }),
      ).throw({
        keyword: "range",
        message:
          '"range.foo": "bar" is not an allowed attribute (allowed: gt,gte,lt,lte)',
        path: "range.foo",
      });
    });
    ...
    it("should reject a field definition with a range keyword not containing a number", () => {
      should(() =>
        koncorde.validate({ range: { foo: { gt: "42", lt: 113 } } }),
      ).throw({
        keyword: "range",
    ...
    it("should reject a field definition containing more than 1 lower boundary", () => {
      should(() =>
        koncorde.validate({ range: { foo: { gt: 42, gte: 13, lt: 113 } } }),
      ).throw({
        keyword: "range",
    ...
    it("should reject a field definition containing more than 1 upper boundary", () => {
      should(() =>
        koncorde.validate({ range: { foo: { gt: 42, lt: 113, lte: 200 } } }),
      ).throw({
        keyword: "range",
    ...
    it("should validate a valid range filter", () => {
      should(() =>
        koncorde.validate({ range: { foo: { gt: 42, lte: 200 } } }),
      ).not.throw();
    });

    it("should reject a range filter with inverted boundaries", () => {
      should(() =>
        koncorde.validate({ range: { foo: { lt: 42, gt: 200 } } }),
      ).throw({
        keyword: "range",
        message:
          '"range.foo": lower boundary must be strictly inferior to the upper one',
        path: "range.foo",
      });
    });
  });
  ...
  describe("#standardization", () => {
    it("should return the same content, unchanged", () => {
      ...
      should(koncorde.transformer.standardizer.standardize(filter)).match(
        filter,
      );
    });
  });
  ...
  describe("#storage", () => {
    it("should store a single condition correctly", () => {
      ...
      const rangeInfo = Array.from(
        store.fields.get("foo").conditions.values(),
      )[0];
    ...
    it("should store multiple conditions on the same field correctly", () => {
      ...
      let rangeInfo = store.fields
        .get("foo")
        .conditions.get(Array.from(sf1.conditions)[0].id);
      should(rangeInfo).instanceOf(RangeCondition);
      should(rangeInfo.subfilters).match(new Set([sf1]));
      should(rangeInfo.low).approximately(42, 1e-9);
      should(rangeInfo.high).approximately(100, 1e-9);

      rangeInfo = store.fields
        .get("foo")
        .conditions.get(Array.from(sf2.conditions)[0].id);
      should(rangeInfo).instanceOf(RangeCondition);
  ...
  describe("#removal", () => {
    ...
    it("should remove a single subfilter from a multi-filter condition", () => {
      ...
      const rangeInfo = Array.from(
        storage.fields.get("foo").conditions.values(),
      )[0];
