...
describe("Koncorde.keyword.geoDistanceRange", () => {
  ...
  describe("#validation/standardization", () => {
    it("should reject an empty filter", () => {
      should(() => standardize({ geoDistanceRange: {} })).throw({
        keyword: "geoDistanceRange",
        message:
          '"geoDistanceRange": expected object to have exactly 3 properties, got 0',
    ...
    it("should reject a filter with multiple field attributes", () => {
      ...
      should(() => standardize({ geoDistanceRange: filter })).throw({
        keyword: "geoDistanceRange",
        message:
          '"geoDistanceRange": expected object to have exactly 3 properties, got 4',
        path: "geoDistanceRange",
      });
    });
    ...
    it("should validate a {lat, lon} point", () => {
      ...
      should(standardize({ geoDistanceRange: filter })).match(
    ...
    it('should validate a {latLon: "geohash"} point', () => {
      ...
      should(result.geospatial.geoDistanceRange.foo.lat).be.approximately(
        point.lat,
        10e-7,
      );

      should(result.geospatial.geoDistanceRange.foo.lon).be.approximately(
        point.lon,
        10e-7,
      );
    });
    ...
    it("should reject a filter with incorrect from value", () => {
      ...
      should(() => standardize({ geoDistanceRange: filter })).throw(
        'unable to parse distance value "1 micronanomillimeter"',
      );
    });
    ...
    it("should reject a filter with incorrect to value", () => {
      ...
      should(() => standardize({ geoDistanceRange: filter })).throw(
        'unable to parse distance value "1 ly"',
      );
    });
    ...
    it("should reject a filter with a negative range", () => {
      ...
      should(() => standardize({ geoDistanceRange: filter })).throw({
        keyword: "geoDistanceRange",
        message:
          '"geoDistanceRange": inner radius must be smaller than outer radius',
        path: "geoDistanceRange",
      });
    });
  });
