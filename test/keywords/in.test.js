...
describe("Koncorde.keyword.in", () => {
  ...
  describe("#validation", () => {
    it("should reject empty filters", () => {
      should(() => koncorde.validate({ in: {} })).throw({
        keyword: "in",
    ...
    it("should reject filters with multiple defined attributes", () => {
      should(() =>
        koncorde.validate({ in: { bar: ["foo"], foo: ["foo"] } }),
      ).throw({
        keyword: "in",
    ...
    it("should reject filters with an empty value list", () => {
      should(() => koncorde.validate({ in: { foo: [] } })).throw({
        keyword: "in",
    ...
    it("should reject filters with non-array values attribute", () => {
      should(() => koncorde.validate({ in: { foo: "foo" } })).throw({
        keyword: "in",
    ...
    it("should reject filters containing a non-string value", () => {
      should(() =>
        koncorde.validate({ in: { foo: ["foo", "bar", 42, "baz"] } }),
      ).throw({
        keyword: "in",
    ...
    it("should validate a well-formed ids filter", () => {
      should(() =>
        koncorde.validate({ in: { foo: ["foo", "bar", "baz"] } }),
      ).not.throw();
    });
  });
