...
describe("Koncorde.keyword.geoBoundingBox", () => {
  ...
  describe("#validation/standardization", () => {
    it("should reject an empty filter", () => {
      should(() => standardize({ geoBoundingBox: {} })).throw({
        keyword: "geoBoundingBox",
        message:
          '"geoBoundingBox": expected object to have exactly 1 property, got 0',
        path: "geoBoundingBox",
      });
    });
    ...
    it("should reject a filter with multiple field attributes", () => {
      should(() =>
        standardize({ geoBoundingBox: { foo: bbox, bar: bbox } }),
      ).throw({
        keyword: "geoBoundingBox",
        message:
          '"geoBoundingBox": expected object to have exactly 1 property, got 2',
        path: "geoBoundingBox",
      });
    });
    ...
    it("should validate a {top, left, bottom, right} bbox", () => {
      ...
      should(standardize({ geoBoundingBox: { foo: box } })).match(
        bboxStandardized,
      );
    });
    ...
    it("should validate a {topLeft: {lat, lon}, bottomRight: {lat, lon}} bbox", () => {
      should(standardize({ geoBoundingBox: { foo: bbox } })).match(
        bboxStandardized,
      );
    });
    ...
    it('should validate a {topLeft: "geohash", bottomRight: "geohash"} bbox', () => {
      ...
      should(result.geospatial.geoBoundingBox.foo.top).be.approximately(
        box.top,
        10e-7,
      );

      should(result.geospatial.geoBoundingBox.foo.bottom).be.approximately(
        box.bottom,
        10e-7,
      );

      should(result.geospatial.geoBoundingBox.foo.left).be.approximately(
        box.left,
        10e-7,
      );

      should(result.geospatial.geoBoundingBox.foo.right).be.approximately(
        box.right,
        10e-7,
      );
    });
  ...
  describe("#storage", () => {
    ...
    it("should add a subfilter to an already existing condition", () => {
      ...
      const id2 = koncorde.register({
        and: [{ geoBoundingBox: { foo: bbox } }, { equals: { foo: "bar" } }],
      });
