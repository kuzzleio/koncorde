...
describe("Koncorde.keyword.notgeospatial", () => {
  ...
  describe("#storage", () => {
    ...
    it("should add another subfilter to an already tested shape", () => {
      ...
      should(storage.fields.get("foo").get(id)).eql(
        new Set([subfilter, subfilter2]),
      );
    });
  });
  ...
  describe("#match", () => {
    ...
    it("should reject a shape if the point is right on its border", () => {
      ...
      should(result.sort()).match(
        [distanceId, distanceRangeId, polygonId].sort(),
      );
    });
  });
  ...
  describe("#removal", () => {
    ...
    it("should remove a single subfilter from a multi-filter condition", () => {
      ...
      should(storage).be.instanceOf(FieldOperand);
      should(
        storage.fields.get("foo").get(Array.from(sf.conditions)[1].id),
      ).match(new Set([sf]));
    });
