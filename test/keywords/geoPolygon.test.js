...
describe("Koncorde.keyword.geoPolygon", () => {
  ...
  beforeEach(() => {
    ...
    standardize = koncorde.transformer.standardizer.standardize.bind(
      koncorde.transformer.standardizer,
    );
  });

  describe("#validation/standardization", () => {
    it("should reject an empty filter", () => {
      should(() => standardize({ geoPolygon: {} })).throw({
        keyword: "geoPolygon",
        message:
          '"geoPolygon": expected object to have exactly 1 property, got 0',
        path: "geoPolygon",
      });
    });
    ...
    it("should reject a filter with multiple field attributes", () => {
      should(() =>
        standardize({ geoPolygon: { foo: polygon, bar: polygon } }),
      ).throw({
        keyword: "geoPolygon",
        message:
          '"geoPolygon": expected object to have exactly 1 property, got 2',
        path: "geoPolygon",
      });
    });
    ...
    it("should reject a filter without a points field", () => {
      ...
      should(() => standardize({ geoPolygon: filter })).throw({
        keyword: "geoPolygon",
    ...
    it("should reject a filter with an empty points field", () => {
      should(() => standardize({ geoPolygon: { foo: { points: [] } } })).throw({
        keyword: "geoPolygon",
        message:
          '"geoPolygon.foo.points": at least 3 points are required to build a polygon',
    ...
    it("should reject a polygon with a non-array points field", () => {
      should(() =>
        standardize({ geoPolygon: { foo: { points: "foobar" } } }),
      ).throw({
        keyword: "geoPolygon",
    ...
    it("should reject a polygon containing an invalid point format", () => {
      ...
      p.push(42);
      should(() => standardize({ geoPolygon: { foo: { points: p } } })).throw({
        keyword: "geoPolygon",
    ...
    it("should standardize all geopoint types in a single points array", () => {
      ...
      result.geospatial.geoPolygon.foo.forEach((p, i) => {
        should(p[0]).be.approximately(
          polygonStandardized.geospatial.geoPolygon.foo[i][0],
          10e-6,
        );
        should(p[1]).be.approximately(
          polygonStandardized.geospatial.geoPolygon.foo[i][1],
          10e-6,
        );
      });
    });
  ...
  describe("#storage", () => {
    it("should store a single geoPolygon correctly", () => {
      ...
      should(
        storage.fields.get("foo").get(Array.from(subfilter.conditions)[0].id),
    ...
    it("should add a subfilter to an already existing condition", () => {
      ...
      const id2 = koncorde.register({
        and: [{ geoPolygon: { foo: polygon } }, { equals: { foo: "bar" } }],
      });
      ...
      should(storage).be.instanceOf(FieldOperand);
      should(
        storage.fields.get("foo").get(Array.from(sf1.conditions)[0].id),
    ...
    it("should add another condition to an already existing field", () => {
      ...
      should(
        storage.fields.get("foo").get(Array.from(sf2.conditions)[0].id),
      ).match(new Set([sf2]));
    });
  });
