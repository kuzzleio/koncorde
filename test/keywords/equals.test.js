...
describe("Koncorde.keyword.equals", () => {
  ...
  describe("#validation", () => {
    it("should reject non-object filters", () => {
      should(() => koncorde.validate({ equals: ["foo", "bar"] })).throw({
        keyword: "equals",
    ...
    it("should reject empty filters", () => {
      should(() => koncorde.validate({ equals: {} })).throw({
        keyword: "equals",
    ...
    it("should reject filters with more than 1 field", () => {
      should(() =>
        koncorde.validate({ equals: { foo: "foo", bar: "bar" } }),
      ).throw({
        keyword: "equals",
    ...
    it("should reject filters with array argument", () => {
      should(() => koncorde.validate({ equals: { foo: ["bar"] } })).throw({
        keyword: "equals",
        message:
          '"equals.foo": must either be a string, a number, a boolean, or null',
        path: "equals.foo",
      });
    });
    ...
    it("should reject filters with object argument", () => {
      should(() => koncorde.validate({ equals: { foo: {} } })).throw({
        keyword: "equals",
        message:
          '"equals.foo": must either be a string, a number, a boolean, or null',
        path: "equals.foo",
    ...
    it("should reject filters with undefined argument", () => {
      should(() => koncorde.validate({ equals: { foo: undefined } })).throw({
        keyword: "equals",
        message:
          '"equals.foo": must either be a string, a number, a boolean, or null',
        path: "equals.foo",
  ...
  describe("#storage", () => {
    ...
    it("should store multiple conditions on the same field correctly", () => {
      ...
      should(equals.fields.get("foo")).have.value(
        "bar",
    ...
    it("should store multiple subfilters on the same condition correctly", () => {
      ...
      const id2 = koncorde.register({
        and: [{ equals: { baz: "qux" } }, { equals: { foo: "bar" } }],
      });
      ...
      should(equals.fields.get("baz")).have.value(
        "qux",
  ...
  describe("#removal", () => {
    ...
    it("should remove a value from the list if its last subfilter is removed", () => {
      ...
      should(equals.fields.get("foo").get("qux")).eql(
        new Set([getSubfilter(id2)]),
      );
