...
describe("Koncorde.keyword.select", () => {
  ...
  describe("#validation", () => {
    it("should reject non-object filters", () => {
      should(() => koncorde.validate({ select: ["foo", "bar"] })).throw({
        keyword: "select",
    ...
    it("should reject empty filters", () => {
      should(() => koncorde.validate({ select: {} })).throw({
        keyword: "select",
        message:
          '"select": expected object to have exactly 3 properties, got 0',
        path: "select",
      });
    });
  ...
  describe("#standardization", () => {
    it("should return the same content, unchanged", () => {
      should(
        koncorde.transformer.standardizer.standardize({
          select: {
            field: "foo",
            index: 0,
            query: {
  ...
  describe("#storage", () => {
    it("should store a single condition correctly", () => {
      ...
      const subfilters = Array.from(
        storage.fields.get("foo").get(0).filters.values(),
      ).flat();
    ...
    it("should store multiple conditions on the same field correctly", () => {
      ...
      should(subfilters.findIndex((f) => f.id === barSubfilter.id)).not.equal(
        -1,
      );
      should(subfilters.findIndex((f) => f.id === quxSubfilter.id)).not.equal(
        -1,
      );
    });
    ...
    it("should store multiple subfilters on the same condition correctly", () => {
      ...
      should(storage).be.instanceOf(FieldOperand);
      should(storage.fields.get("baz")).instanceOf(Map);
      should(storage.fields.get("baz").get(0).filters.size).equal(1);
      let subfilters = Array.from(
        storage.fields.get("baz").get(0).filters.values(),
      ).flat();
      should(subfilters.findIndex((f) => f.id === multiSubfilter.id)).not.equal(
        -1,
      );
  ...
  describe("#matching", () => {
    ...
    it("should not match if the document contains the field with another value", () => {
      ...
      should(koncorde.test({ foo: ["bar", "qux"] }))
        .be.an.Array()
        .and.be.empty();
    });
    ...
    it("should skip the matching if the document tested property is not of the same type than the known values", () => {
      ...
      should(koncorde.test({ foo: { bar: true } }))
        .be.an.Array()
        .and.empty();
    });
  ...
  describe("#removal", () => {
    ...
    it("should remove a value from the list if its last subfilter is removed", () => {
      ...
      subfilters = Array.from(
        storage.fields.get("foo").get(1).filters.values(),
      ).flat();
      should(subfilters.length).equal(1);
      should(subfilters.findIndex((f) => f.id === quxSubfilter.id)).not.equal(
        -1,
