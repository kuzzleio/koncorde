...
describe("Koncorde.keyword.ids", () => {
  ...
  describe("#validation", () => {
    it("should reject empty filters", () => {
      should(() => koncorde.validate({ ids: {} })).throw({
        keyword: "ids",
    ...
    it('should reject filters with other fields other than the "values" one', () => {
      should(() => koncorde.validate({ ids: { foo: ["foo"] } })).throw({
        keyword: "ids",
    ...
    it("should reject filters with multiple defined attributes", () => {
      should(() =>
        koncorde.validate({ ids: { values: ["foo"], foo: ["foo"] } }),
      ).throw({
        keyword: "ids",
    ...
    it("should reject filters with an empty value list", () => {
      should(() => koncorde.validate({ ids: { values: [] } })).throw({
        keyword: "ids",
    ...
    it("should reject filters with non-array values attribute", () => {
      should(() => koncorde.validate({ ids: { values: "foo" } })).throw({
        keyword: "ids",
    ...
    it("should reject filters containing a non-string value", () => {
      should(() =>
        koncorde.validate({ ids: { values: ["foo", "bar", 42, "baz"] } }),
      ).throw({
        keyword: "ids",
    ...
    it("should validate a well-formed ids filter", () => {
      should(() =>
        koncorde.validate({ ids: { values: ["foo", "bar", "baz"] } }),
      ).not.throw();
    });
  });
