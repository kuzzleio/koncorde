...
describe("Koncorde.keyword.geoDistance", () => {
  ...
  describe("#validation/standardization", () => {
    it("should reject an empty filter", () => {
      should(() => standardize({ geoDistance: {} })).throw({
        keyword: "geoDistance",
        message:
          '"geoDistance": expected object to have exactly 2 properties, got 0',
        path: "geoDistance",
      });
    });
    ...
    it("should reject a filter with multiple field attributes", () => {
      ...
      should(() => standardize(filter)).throw({
        keyword: "geoDistance",
        message:
          '"geoDistance": expected object to have exactly 2 properties, got 3',
        path: "geoDistance",
      });
    });
    ...
    it("should validate a {lat, lon} point", () => {
      should(
        standardize({ geoDistance: { foo: point, distance: "1km" } }),
    ...
    it("should validate a {latLon: [lat, lon]} point", () => {
      ...
      should(standardize({ geoDistance: { foo: p, distance: "1km" } })).match(
    ...
    it('should validate a {latLon: "geohash"} point', () => {
      ...
      should(result.geospatial.geoDistance.foo.lat).be.approximately(
        point.lat,
        10e-7,
      );

      should(result.geospatial.geoDistance.foo.lon).be.approximately(
        point.lon,
        10e-7,
      );
    });
    ...
    it("should reject a filter with a non-string distance value", () => {
      should(() =>
        standardize({ geoDistance: { foo: point, distance: 42 } }),
      ).throw({
        keyword: "geoDistance",
    ...
    it("should reject a filter with incorrect distance value", () => {
      should(() =>
        standardize({ geoDistance: { foo: point, distance: "1 ly" } }),
