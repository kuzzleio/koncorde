...
describe("Koncorde API", () => {
  ...
  describe("#constructor", () => {
    it("should throw if an invalid argument is supplied", () => {
      should(() => new Koncorde("foobar")).throw(
        /Invalid argument: expected an object/,
      );
      should(() => new Koncorde(["foo", "bar"])).throw(
        /Invalid argument: expected an object/,
      );
  ...
  describe("#validate", () => {
    ...
    it("should reject if a filter is not valid", () => {
      should(() =>
        koncorde.validate({ equals: { foo: "bar" }, exists: "qux" }),
      ).throw();
    });
  });
  ...
  describe("#register", () => {
    ...
    it("should resolve to the same id for equivalent filters", () => {
      ...
      const id2 = koncorde.register({
        or: [{ not: { exists: "bar" } }, { not: { equals: { foo: "bar" } } }],
      });
  ...
  describe("#test", () => {
    ...
    it("should behave correctly on complex filters", () => {
      ...
      should(koncorde.test({ foo: "qux", num: 1 }))
        .Array()
        .and.empty();
      should(koncorde.test({ num: 41 }))
        .Array()
        .and.empty();
      should(koncorde.test({ foo: "bar", num: 41 }))
        .Array()
        .and.empty();
  ...
  describe("#remove", () => {
    ...
    it("should unsubscribe a filter from a multi-filter subfilter", () => {
      ...
      ids.push(
        koncorde.register({
          or: [{ equals: { foo: "bar" } }, { exists: { field: "bar" } }],
        }),
      );
  ...
  describe("#normalize", () => {
    it("should invoke the normalizer", () => {
      const f = {
        not: {
          and: [{ exists: { field: "bar" } }, { equals: { foo: "bar" } }],
        },
      };
      const n = [];
    ...
    it("should throw if an invalid index name is provided", () => {
      for (const ohnoes of [true, false, 0, 123, {}, []]) {
        ...
        should(() => koncorde.normalize({}, ohnoes)).throw(
          'Invalid "index" argument: must be a string',
        );
      }
    });
  });
  ...
  describe("#store", () => {
    ...
    it("should throw when receiving a non-NormalizedFilter object", () => {
      should(() => koncorde.store({})).throw(/Invalid argument/);
    });
