...
describe("koncorde.operands.and", () => {
  ...
  describe("#validation", () => {
    it("should reject empty filters", () => {
      should(() => koncorde.validate({ and: [] })).throw({
        keyword: "and",
    ...
    it("should reject non-array content", () => {
      should(() => koncorde.validate({ and: { foo: "bar" } })).throw({
        keyword: "and",
    ...
    it("should reject if one of the content is not an object", () => {
      const filter = {
        and: [{ equals: { foo: "bar" } }, [{ exists: { field: "foo" } }]],
      };
    ...
    it("should reject if one of the content object is not a well-formed keyword", () => {
      const filter = {
        and: [{ equals: { foo: "bar" } }, { exists: { foo: "bar" } }],
      };
    ...
    it('should validate a well-formed "and" operand', () => {
      const filter = {
        and: [{ equals: { foo: "bar" } }, { exists: { field: "bar" } }],
      };
