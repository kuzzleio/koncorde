...
class Standardizer {
  ...
  regexp(filter, path) {
    ...
    if (typeof filter.regexp[field] === "string") {
      ...
      Object.prototype.toString.call(filter.regexp[field]) === "[object Object]"
    ) {
      const fieldPath = pathAdd(path, field);
  ...
  select(filter, path) {
    ...
    isObject(filter.select, "query", "select", pathAdd(path, "query"), {
      nonEmpty: true,
    });
  ...
  geoBoundingBox(filter, path) {
    ...
    if (
      BBOX_PROPERTIES.every((v) =>
        ["string", "number"].includes(typeof bBox[v]),
      )
    ) {
    ...
    if (
      BBOX_PROPERTIES.some(
        (v) => standardized[v] === null || standardized[v] === undefined,
      )
    ) {
      throw new KoncordeParseError(
  ...
  geoDistanceRange(filter, path) {
    ...
    const field = Object.keys(filter.geoDistanceRange).find(
      (f) => f !== "from" && f !== "to",
    );

    const [from, to] = ["from", "to"].map((f) => {
      isString(
        filter.geoDistanceRange,
        f,
        "geoDistanceRange",
        pathAdd(path, f),
      );
  ...
  _standardizeFilterArray(filter, operand, path) {
    ...
    const idx = Object.keys(filter[operand]).findIndex((v) => {
      return (
        Object.prototype.toString.call(filter[operand][v]) !==
          "[object Object]" || Object.keys(filter[operand][v]).length === 0
      );
    });
...
function isInteger(filter, property, keyword, path) {
  ...
  if (Math.floor(filter[property]) !== filter[property]) {
    throw new KoncordeParseError(
      "cannot have decimals, must be an integer",
      keyword,
      path,
    );
  }
...
function isArray(filter, property, keyword, path, options = null) {
  ...
  if (options) {
    ...
    if (
      options.type &&
      !filter[property].every((i) => typeof i === options.type)
    ) {
      throw new KoncordeParseError(
