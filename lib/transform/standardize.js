...
class Standardizer {
   ...
   */
  match (filter, path) {
    isObject(filter, 'match', 'match', path, { properties: 1 });

    const field = Object.keys(filter.match)[0];

    if (filter.match[field] === null || typeof filter.match[field] !== 'object') {
      throw new KoncordeParseError('must be an object or an array', 'match', pathAdd(path, field));
    }
