...
const
  ...
  NormalizedExists = require('./normalizedExists');
...
class Standardizer {
  ...
  exists (filter, name = 'exists') {
    ...
    if (typeof filter[name] === 'string') {
      if (filter[name].length === 0) {
        return Bluebird.reject(new BadRequestError(`${name}: cannot test empty field name`));
      }
  ...
  geoBoundingBox (filter) {
    return mustBeNonEmptyObject(filter.geoBoundingBox, 'geoBoundingBox')
      ...
      .then(field => {
        ...
        if (['top', 'left', 'bottom', 'right'].every(v => ['string', 'number'].includes(typeof bBox[v]))) {
          for (const v of ['top', 'left', 'bottom', 'right']) {
        ...
        // { topLeft: geopoint, bottomRight: geopoint } (see convertGeopoint)
          ...
          const
            topLeft = convertGeopoint(bBox.topLeft),
            bottomRight = convertGeopoint(bBox.bottomRight);
        ...
        if (['top', 'left', 'bottom', 'right'].some(v => standardized[v] === null || standardized[v] === undefined)) {
  ...
  geoPolygon (filter) {
    ...
    return mustBeNonEmptyObject(filter.geoPolygon, 'geoPolygon')
      .then(fields => onlyOneFieldAttribute(fields, 'geoPolygon'))
      .then(fields => {
        docField = fields[0];
...
function parseFieldSyntax(field, keyword) {
  ...
  if (array) {
    ...
    try {
      ...
      return Bluebird.reject(new BadRequestError(`[${keyword}] Invalid array value "${rawValue}"`));
    }
  }
  ...
  return Bluebird.resolve({[keyword]: new NormalizedExists(path, array, value)});
}
