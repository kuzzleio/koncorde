...
class TestTables {
  constructor(testTablesRef, index, collection) {
    ...
    this.matchedConditions = new Map();
  ...
  addMatch(subfilters) {
    ...
    for (i = 0; i < subfilters.length; i++) {
      ...
      const matched = this.matchedConditions.get(sf.cidx) || this.conditions[sf.cidx];

      if (matched > 1) {
        this.matchedConditions.set(sf.cidx, matched - 1);
      } else {
        ...
        for (j = 0; j < sf.filters.length; j++) {
          this.matched.add(sf.filters[j].id);
        }
      }
    }
  }
