 ...
 */
function MatchNotRegexp (operand, testTables, document) {
  for (const key of operand.keys) {
    for (const entry of operand.fields[key].values()) {
      if (document[key] === undefined || !entry.regexp.test(document[key])) {
        testTables.addMatch(entry.subfilters);
      }
    }
  }
}
