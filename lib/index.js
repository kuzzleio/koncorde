...
class RealtimeEngine {
  ...
  constructor(config) {
    ...
    if (typeof this.config !== 'object' || Array.isArray(this.config)) {
      throw new Error(`Invalid argument: expected an object, got ${typeof this.config}`);
    }
  ...
  normalize(index, collection, filters) {
    return this.transformer.normalize(filters)
      .then(normalized => ({
        ...
        collection,
        normalized, 
        id: this.storage.getRoomId(index, collection, normalized)
      }));
  }
   ...
   *
   * @param {string} roomId - ID of the room to remove
   * @returns {Promise}
   */
  remove(roomId) {
    return this.storage.remove(roomId);
  }
}
