...
class OperandsStorage {
  ...
  regexp(foPairs, subfilter, condition) {
    ...
    } else if ((entries = foPairs.fields[fieldName].get(value.stringValue)) !== undefined) {
      entries.push(subfilter);
    } else {
  ...
  notgeospatial(foPairs, subfilter, condition) {
    ...
    if (!foPairs.fields[fieldName]) {
      foPairs.keys.push(fieldName);
      foPairs.fields[fieldName] = new Map([[condition.id, [subfilter]]]);
    } else if ((entries = foPairs.fields[fieldName].get(condition.id)) !== undefined) {
      entries.push(subfilter);

      // skip the shape insertion in the geospatial index
      return;
    } else {
      foPairs.fields[fieldName].set(condition.id, [subfilter]);
    }
