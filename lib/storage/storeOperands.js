...
class OperandsStorage {
  ...
  equals(operand, subfilter, condition) {
    ...
    if (!operand.fields[fieldName]) {
      operand.keys.add(fieldName);
      operand.fields[fieldName] = new Map([[value, new Set([subfilter])]]);
    } else if ((entries = operand.fields[fieldName].get(value)) === undefined) {
      operand.fields[fieldName].set(value, new Set([subfilter]));
    } else {
  ...
  exists(operand, subfilter, condition) {
    ...
    if (!operand.fields[path]) {
      operand.keys.add(path);
      operand.fields[path] = {
        subfilters: new Set(),
        values: new Map()
      };
    ...
    if (!condition.value.array) {
      operand.fields[path].subfilters.add(subfilter);
    } else if ((entries = operand.fields[path].values.get(value)) !== undefined) {
      entries.add(subfilter);
    } else {
      operand.fields[path].values.set(value, new Set([subfilter]));
    }
  }
  ...
  range(operand, subfilter, condition) {
    ...
    if (!operand.fields[field]) {
      operand.keys.add(field);
      operand.fields[field] = {
        tree: new IntervalTree(),
        conditions: new Map([[condition.id, rangeCondition]])
      };
    }
    else if ((entry = operand.fields[field].conditions.get(condition.id)) !== undefined) {
      entry.subfilters.add(subfilter);
    }
    else {
      operand.fields[field].conditions.set(condition.id, rangeCondition);
    }

    if (!entry) {
      operand.fields[field].tree.insert(
        rangeCondition.low,
        rangeCondition.high,
        rangeCondition);
    }
  ...
  notrange(operand, subfilter, condition) {
    ...
    if (!entry) {
      if (rangeCondition.low !== -Infinity) {
        operand.fields[field].tree.insert(-Infinity, rangeCondition.low, rangeCondition);
      }

      if (rangeCondition.high !== Infinity) {
        operand.fields[field].tree.insert(rangeCondition.high, Infinity, rangeCondition);
      }
    }
  }
  ...
  regexp(operand, subfilter, condition) {
    ...
    if (!operand.fields[fieldName]) {
      operand.keys.add(fieldName);
      operand.fields[fieldName] = new Map([[value.stringValue, value]]);
    } else if ((entry = operand.fields[fieldName].get(value.stringValue)) !== undefined) {
      entry.subfilters.add(subfilter);
    } else {
      operand.fields[fieldName].set(value.stringValue, value);
    }
  }
  ...
  geospatial(operand, subfilter, condition) {
    ...
    if (!operand.fields[fieldName]) {
      operand.keys.add(fieldName);
      operand.fields[fieldName] = new Map([[condition.id, new Set([subfilter])]]);
    } else if ((subfilters = operand.fields[fieldName].get(condition.id)) !== undefined) {
      subfilters.add(subfilter);

      // skip the shape insertion in the geospatial index
      return;
    } else {
      operand.fields[fieldName].set(condition.id, new Set([subfilter]));
    }
