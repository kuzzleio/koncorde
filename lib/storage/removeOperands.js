...
class OperandsRemoval {
  ...
  range(foPairs, index, collection, subfilter, condition) {
    const
      ...
      fieldName = Object.keys(condition.value)[0],
      field = operand.fields.get(fieldName),
      rangeCondition = field.conditions.get(condition.id);
  ...
  regexp(foPairs, index, collection, subfilter, condition, keyword = 'regexp') {
    const
      fieldName = Object.keys(condition.value)[0],
      reCondition = new RegExpCondition(
        this.config,
        condition.value[fieldName].value,
        null,
        condition.value[fieldName].flags),
      stringValue = reCondition.stringValue,
      operand = foPairs.get(index, collection, keyword),
      field = operand.fields.get(fieldName),
      regexpCondition = field.get(stringValue);
