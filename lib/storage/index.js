...
class Storage {
   ...
   * @param {Array} filters
   * @param {String} [roomId] Optional: may be computed beforehand with getRoomId
   * @return {object}
   */
  store (index, collection, filters, roomId = null) {
    const
      result = this._addFilter(index, collection, filters, roomId),
      response = {
        id: result.id,
        diff: {
          index,
  ...
  remove (roomId) {
    ...
    const {index, collection} = this.filters[roomId];

    this._removeFromTestTables(index, collection, this.filters[roomId]);

    let i; // NOSONAR

    for (i = 0; i < this.filters[roomId].subfilters.length; i++) {
      const subfilter = this.filters[roomId].subfilters[i];
      ...
      if (subfilter.filters.length === 1) {
        ...
        subfilter.filters.splice(subfilter.filters.indexOf(this.filters[roomId]), 1);
      }
    }
    ...
    if (this.filtersIndex[index][collection].length === 1) {
      ...
      this.filtersIndex[index][collection].splice(this.filtersIndex[index][collection].indexOf(roomId), 1);
    }

    delete this.filters[roomId];
  }
   ...
   * @param {string} [roomId] Filter unique identifier
   * @return {object} containing a "created" boolean flag and the filter id
   */
  _addFilter (index, collection, filters, roomId = null) {
    const 
      id = roomId || this.getRoomId(index, collection, filters),
  ...
  _reindexTestTable (index, collection) {
    ...
    if (tt.removedFilters.size > 0) {
      ...
      for (i = 0; i < this.filtersIndex[index][collection].length; i++) {
        const
          roomId = this.filtersIndex[index][collection][i],
          filter = this.filters[roomId];
   ...
   */
  getRoomId (index, collection, filters) {
