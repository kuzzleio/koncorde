...
const { CacheSet, CacheMap } = require('./objects/cacheStorage');
...
class Storage {
  ...
  getFilterId (index, collection, filters) {
    return this.hash({ collection, filters, index });
  }
  ...
  store (index, collection, filters, filterId = null) {
    ...
    for(let i = 0; i < filters.length; i++) {
      ...
      if (sfResult.created) {
        const subfilter = this.subfilters.get(index, collection, sfResult.id);
        const addedConditions = this._addConditions(
          subfilter,
          index,
          collection,
          sf);
  ...
  _addFilter (index, collection, filters, filterId = null) {
    const id = filterId || this.getFilterId(index, collection, filters);
  ...
  _addConditions (subfilter, index, collection, conditions) {
    ...
    for(let i = 0; i < conditions.length; i++) {
      ...
      const condLink = this.conditions.get(index, collection, cId);
